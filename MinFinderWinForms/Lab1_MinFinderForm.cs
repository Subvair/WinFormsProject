...
{
    public partial class Lab1_MinFinderForm : Form
    {
        public Lab1_MinFinderForm()
        {
            InitializeComponent();
            InitChart();
        ...
        private void Calculate()
        {
            ...
            var (xMin, fMin, iters) = DichotomyMin(a, b, e, fx);
        ...
        private bool TryReadInputs(out double a, out double b, out double e, out string fx)
        {
            ...
            if (e >= (b - a)) { Warn("Точность e слишком большая для данного отрезка (e < b-a).", txtE); return false; }
        ...
        private bool TryEval(string fx, double x, out double y, out string error)
        {
            ...
            try
            {
                ...
                {
                    error = "Получено NaN/Infinity (возможное деление на 0 или неверная область определения).";
                    return false;
                }
                return true;
            }
        ...
        private (double xMin, double fMin, int iters) DichotomyMin(double a, double b, double eps, string fx)
        {
            int iters = 0;

            // delta должна быть меньше eps, чтобы точки были "рядом"
